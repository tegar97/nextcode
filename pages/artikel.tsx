...
function Artikel({tagData}) {
  ...
  // });
   ...
   const settings  = {
    ...
    const { data, isError,isLoading} = usePost(`https://blog-backend.tegar.me/ghost/api/v3/content/posts/?key=8bc89b54fa72341924f8558631&${search ? '' : 'limit=8'}&include=tags&${tag ? `filter=tag:${tag}` : ''}&page=${page}`)
    return (
      <>
       <NextSeo 
